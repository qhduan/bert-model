...
flags.DEFINE_string(
    ...
    "Name of the checkpoint under bert_directory to be exported.")
...
def get_mlm_logits(model, bert_config, mlm_positions):
...
def module_fn(is_training):
  ...
  # mlm_logits = get_mlm_logits(model, bert_config, mlm_positions)

  # vocab_model_path = os.path.join(FLAGS.bert_directory, "30k-clean.model")
  # vocab_file_path = os.path.join(FLAGS.bert_directory, "30k-clean.vocab")

  config_file = tf.constant(
      value=bert_config_path, dtype=tf.string, name="config_file")
...
def main(_):
  ...
  checkpoint_path = os.path.join(FLAGS.bert_directory, FLAGS.checkpoint_name)
